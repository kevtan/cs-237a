...
class CameraCalibrator:
    ...
    def genCornerCoordinates(self, u_meas, v_meas):
        ...
        '''
        EPSILON = 0.001
        x_coords = np.arange(0, self.n_corners_x * self.d_square - EPSILON, self.d_square)
        y_coords = np.arange(0, self.n_corners_y * self.d_square - EPSILON, self.d_square)
    ...
    def estimateHomography(self, u_meas, v_meas, X, Y):    # Zhang Appendix A
        ...
        def constraints(M_tilde, u, v):
            ...
            return row1, row2
        rows = map(lambda triple: constraints(*triple), zip(M_tildes, u_meas, v_meas))
    ...
    def getExtrinsics(self, H, A):
        ...
        R = np.concatenate(tuple(map(lambda col: col.reshape(3, 1), [r1, r2, r3])), axis=1)
